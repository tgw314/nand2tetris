...
func (ce *CompilationEngine) compileOr() {
	ce.compileAnd()
	for ce.tok.ConsumeSym('|') {
		ce.compileAnd()
...
func (ce *CompilationEngine) compileAnd() {
	ce.compileEquality()
	for ce.tok.ConsumeSym('&') {
		ce.compileEquality()
...
func (ce *CompilationEngine) compileEquality() {
	ce.compileRelational()
	for ce.tok.ConsumeSym('=') {
		ce.compileRelational()
...
func (ce *CompilationEngine) compileRelational() {
	ce.compileAdd()
	for {
		if ce.tok.ConsumeSym('<') {
			ce.compileAdd()
			ce.vm.Arithmetic(vm.LT)
		} else if ce.tok.ConsumeSym('>') {
			ce.compileAdd()
...
func (ce *CompilationEngine) compileAdd() {
	ce.compileMul()
	for {
		if ce.tok.ConsumeSym('+') {
			ce.compileMul()
...
func (ce *CompilationEngine) compileMul() {
	ce.compileUnary()
	for {
		if ce.tok.ConsumeSym('*') {
			ce.compileUnary()
...
func (ce *CompilationEngine) compileUnary() {
	if ce.tok.ConsumeSym('-') {
		ce.compileTerm()
